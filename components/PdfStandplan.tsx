...
const sectionInfo: Record<Section, { title: string; text: string; color: string; path: string }> = {
  A: {
    title: "Passage",
...
export default function PdfStandplan() {
  ...
  useEffect(() => {
    ...
    const positionPopup = (target: SVGPathElement) => {
      const wrap = wrapRef.current;
      if (!wrap) return;
      const wrapRect = wrap.getBoundingClientRect();
      const areaRect = target.getBoundingClientRect();
      const centerX = areaRect.left + areaRect.width / 2 - wrapRect.left;
      const centerY = areaRect.top + areaRect.height / 2 - wrapRect.top;
      const left = Math.min(Math.max(centerX, 150), Math.max(150, wrapRect.width - 150));
      const top = Math.min(Math.max(centerY, 90), Math.max(90, wrapRect.height - 90));
    ...
    const handlePointerOver = (event: PointerEvent) => {
      ...
      if (!area) return;
      setHoveredArea(area);
      positionPopup(event.target as SVGPathElement);
    };
    ...
    const handlePointerOut = (event: PointerEvent) => {
      ...
      if (related instanceof Element && related.closest?.(".areaHit") === event.target) return;
      setHoveredArea(null);
    };
  ...
  return (
    <div ref={wrapRef} className="pdfStandplan" aria-hidden="true">
      ...
      {hoveredArea && hoverInfo && (
        <>
          <svg className="hoverFrost" viewBox={`0 0 ${PAGE_WIDTH} ${PAGE_HEIGHT}`} preserveAspectRatio="none">
            <defs>
            ...
            <foreignObject x="0" y="0" width={PAGE_WIDTH} height={PAGE_HEIGHT} mask="url(#hoverFocusMask)">
              <div className="hoverFrostLayer" />
            </foreignObject>
          </svg>

          <div
            className={`areaHoverPopup area-${hoveredArea.toLowerCase()}`}
            style={{ left: popupPosition.left, top: popupPosition.top, borderColor: hoverInfo.color }}
          >
            <span className="areaHoverKicker">Bereich {hoveredArea}</span>
